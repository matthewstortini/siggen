...
cdef class PySiggen_PPC:
  cdef Setup* setup
  cdef Detector[PPC]* detector
  cdef SignalGenerator[PPC]* siggen

  def __cinit__(self, conf_file):
    self.setup = new Setup(conf_file.encode('utf-8'))
    self.detector = new Detector[PPC](dereference(self.setup))
    self.siggen = new SignalGenerator[PPC](self.detector, dereference(self.setup))
